 ...
 * 后面再用的时候都不用再调用服务。这样虽然避免了多次调用服务，减少了资源占用，提升了性能，
 * 但是增加了耦合性（启动时的时序耦合）。
 * 使用该工具后的效果，可以保证一些操作最多只执行一次。并且不用在启动时执行。
 * 不过这要对代码稍微调整一下。
...
public class Defer {
	 ...
	 * @param cb
	 */
	public static void once(String key,Callback cb){
	...
	public static void once(Class<?> klass,Callback cb){
